...
try:
    ...
    print(
        f"Unable to read key file at {keypath}! Must be a PEM-encoded ECDSA private key."
    )
...
def sign_sslib(artifact):
    ...
    envelope = metadata.Envelope(
        payload=artifact, payload_type=payloadType, signatures=[]
    )
...
result = dsse.Verify(
    json.dumps(sslib_envelope).encode(), [("mykey_ecdsa", dsse_lib_verifier)]
)
print(result)
